...
int main()
{
	...
	RenderWindow window(VideoMode(), "The Game!", Style::Fullscreen);
	Vector2u windowSize = window.getSize();

	Texture t1, t2, t3;
	t1.loadFromFile("tiles2.png");
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < M; i++)
			for (int j = 0; j < N; j++)
			{
				...
				s.move((windowSize.x - tileSize * N) / 2, 31); //offset
				window.draw(s);
			}
